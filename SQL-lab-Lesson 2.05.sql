...
-- assuming since first rental date to last , 275 days
...
-- 9. Get release years.
SELECT DISTINCT release_year FROM film;  -- only 2006;

-- 10. Get all films with ARMAGEDDON in the title.
SELECT * FROM film WHERE title LIKE '%ARMAGEDDON%';   -- there is no film called Armageddon
...
SELECT * FROM film;
SELECT COUNT(title) FROM film WHERE special_features LIKE '%Behind the Scenes%';   -- 538 films include